...
self.onmessage = function ( e ) {
	...
	if ( data.init ) {
		...
		if ( ! renderer ) renderer = new RaytracingRendererWorker();
...
const RaytracingRendererWorker = function () {

	console.log( 'RaytracingRendererWorker', REVISION );
	...
	var raycasterLight = new Raycaster();
	var rayLight = raycasterLight.ray;

	var perspective;
	var cameraNormalMatrix = new Matrix3();
	...
	this.render = function ( scene, camera ) {
		...
		perspective = 0.5 / Math.tan( Math.degToRad( camera.fov * 0.5 ) ) * canvasHeight;
...
Object.assign( RaytracingRendererWorker.prototype, EventDispatcher.prototype );

export default RaytracingRendererWorker;
