import { Box3, Group, Math, Vector3 } from 'three';
	...
	Car.prototype = {
		...
		update: function ( delta ) {
			...
			if ( controls.moveForward ) {
				...
				this.speed = Math.clamp( this.speed + delta * this.acceleration, maxSpeedReverse, this.maxSpeed );
				acceleration = Math.clamp( acceleration + delta, - 1, 1 );

			}

			if ( controls.moveBackward ) {

				this.speed = Math.clamp( this.speed - delta * accelerationReverse, maxSpeedReverse, this.maxSpeed );
				acceleration = Math.clamp( acceleration - delta, - 1, 1 );
			...
			if ( controls.moveLeft ) {
				...
				wheelOrientation = Math.clamp( wheelOrientation + delta * steeringWheelSpeed, - maxSteeringRotation, maxSteeringRotation );
			...
			if ( ! ( controls.moveForward || controls.moveBackward ) ) {
				...
				if ( this.speed > 0 ) {
					...
					this.speed = Math.clamp( this.speed - k * delta * deceleration * brakingDeceleration, 0, this.maxSpeed );
					acceleration = Math.clamp( acceleration - k * delta, 0, 1 );

				} else {

					var k = exponentialEaseOut( this.speed / maxSpeedReverse );

					this.speed = Math.clamp( this.speed + k * delta * accelerationReverse * brakingDeceleration, maxSpeedReverse, 0 );
					acceleration = Math.clamp( acceleration + k * delta, - 1, 0 );
			...
			if ( ! ( controls.moveLeft || controls.moveRight ) ) {
				...
				if ( wheelOrientation > 0 ) {
					...
					wheelOrientation = Math.clamp( wheelOrientation - delta * steeringWheelSpeed, 0, maxSteeringRotation );

				} else {

					wheelOrientation = Math.clamp( wheelOrientation + delta * steeringWheelSpeed, - maxSteeringRotation, 0 );
