import { Material, Math, Mesh, MeshBasicMaterial, Object3D } from 'three';
...
const LightningStorm = function ( stormParams ) {
	...
	if ( stormParams.onRayPosition !== undefined ) {
		...
		this.onRayPosition = function( source, dest ) {
			...
			var height = Math.lerp( stormParams.minHeight, stormParams.maxHeight, Math.random() );;
...
LightningStorm.prototype.update = function ( time ) {
	...
	if ( time >= this.nextLightningTime ) {
		...
		if ( lightningMesh ) {
			...
			lightningParams1.deathTime = time + Math.lerp( this.stormParams.lightningMinDuration, this.stormParams.lightningMaxDuration, Math.random() );
...
LightningStorm.prototype.getNextLightningTime = function ( currentTime ) {
	...
	return currentTime + Math.lerp( this.stormParams.lightningMinPeriod, this.stormParams.lightningMaxPeriod, Math.random() ) / ( this.stormParams.maxLightnings + 1 );
